...
def get_crypto_kline(crypto_name: str, time_interval: str = "15m") -> MessageSegment:
    ...
    time_interval_repl = {"15m": "15m", "1h": "1H", "4h": "4H", "1D": "1Dutc", "3D": "3Dutc", "1W": "1Wutc"}
    ...
    fig.savefig(os.getcwd() + f"/src/data/crypto/kline_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
    return MessageSegment.image("file:///" + os.getcwd() + f"/src/data/crypto/kline_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
