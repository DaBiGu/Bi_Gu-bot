...
def get_user_data(username: str) -> MessageSegment:
    ...
    fig.text(0.8, 0.03, "\u00A9" + f"Generated by Bi_Gu-bot at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", 
             ha='center', fontsize = 15)
    ax.set_ylim(min_score - 10000, max_score + 10000)
    ax.set_yticks(np.arange(int(min_score // 10000 + 1) * 10000, int(max_score // 10000 + 1) * 10000, 10000))
    ...
    for i in range(len(pt_count)):
        ...
        if delta_pt > 0:
            pt_plus.append(delta_pt)
            pt_minus.append(0)
        else:
            pt_plus.append(0)
            pt_minus.append(delta_pt)
