...
def search_game(game_name: str) -> List[Dict[str, str]]:
    ...
    if response:
        for item in response:
            ...
            with open(f"D:/Bi_Gu-bot/Bi_Gu-bot/src/data/steam/temp/{name}.png", "wb") as f:
                f.write(requests.get(image_url).content)
            current_price, regular_price, history_low = [price_info[0]["deals"][0][x]["amount"] for x in ["price", "regular", "storeLow"]]
            discount = price_info[0]["deals"][0]["cut"]
            game = {"name": name, "banner_path": f"D:/Bi_Gu-bot/Bi_Gu-bot/src/data/steam/temp/{name}.png", 
                    "current_price": f"{current_price} CNY", "regular_price": f"{regular_price} CNY", "history_low": f"{history_low} CNY", "discount": f"-{discount}%"}
            games.append(game)
    return games
...
def draw_search_result(game_name: str) -> MessageSegment:
    ...
    font_path = os.getcwd() + "/src/data/steam/NotoSans-Regular.ttf"
    bold_font_path = os.getcwd() + "/src/data/steam/NotoSans-Bold.ttf"
    semibold_font_path = os.getcwd() + "/src/data/steam/NotoSans-SemiBold.ttf"
    font_size = 30
    ...
    total_height = len(games) * (banner_size[1] + 2 * padding)
    ...
    image.save(os.getcwd() + "/src/data/steam/search_result.png")
    return MessageSegment.image("file:///" + os.getcwd() + "/src/data/steam/search_result.png")